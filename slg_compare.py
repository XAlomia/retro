...
#
read_df = \
pd.read_csv("C:/Users/Xavier/Documents/retro/all2019.csv", header = None)
...
df['empty_bs'] = np.where(df.base1_run_id.isnull() & \
  df.base2_run_id.isnull() & df.base3_run_id.isnull(), 1, 0)
    
df = df.query("year == '2019'")
df = df.groupby(by = 'empty_bs').agg({'h':'sum','ab':'sum','sngl':'sum','dbl':\
               'sum','trpl':'sum','hr':'sum'})
#df = df.query("ab > 500")
...
et = int(df.query("empty_bs == 1").slg.values[0] * \
         df.query("empty_bs == 1").ab.values[0])
net = int(df.query("empty_bs == 0").slg.values[0] * \
         df.query("empty_bs == 0").ab.values[0])
ep = df.query("empty_bs == 1").ab.values[0]
nep = df.query("empty_bs == 0").ab.values[0]
